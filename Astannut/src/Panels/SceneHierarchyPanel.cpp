		...
		DrawComponent<CameraComponent>("Camera", entity, [](auto& component)
			{
				...
				if (camera.GetProjectionType() == SceneCamera::ProjectionType::Perspective)
				{
					...
					float orthoNear = camera.GetPerspectiveVerticalNearClip();
					if (ImGui::DragFloat("Near", &orthoNear))
						camera.SetPerspectiveVerticalNearClip(orthoNear);

					float orthoFar = camera.GetPerspectiveVerticalFarClip();
					if (ImGui::DragFloat("Far", &orthoFar))
						camera.SetPerspectiveVerticalFarClip(orthoFar);
				}
		...
		DrawComponent<ScriptComponent>("Script", entity, [entity, this](auto& component) mutable
			{
				...
				else
				{
					if (scriptClassExits)
					{
						...
						const auto& fileds = entityClass->GetFields();

						auto& entityFields = ScriptEngine::GetScriptFieldMap(entity);
						for (const auto& [name, field] : fileds)
						{
							...
							if (entityFields.find(name) != entityFields.end())
							{
								...
								if (field.Type == ScriptFieldType::Float)
								{
									...
									if (ImGui::DragFloat(name.c_str(), &data))
										entityFields.at(name).SetValue<float>(data);
								}
							}
							else
							{
