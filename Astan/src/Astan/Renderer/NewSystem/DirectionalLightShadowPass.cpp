...
namespace Astan
{
    ...
    void DirectionalLightShadowPass::SetupAttachments()
    {
        ...
        m_RenderCommand->CreateImage(s_directional_light_shadow_map_dimension,
    ...
    void DirectionalLightShadowPass::SetupRenderPass()
    {
        ...
        if (RHI_SUCCESS != m_RenderCommand->CreateRenderPass(&renderpass_create_info, m_FrameBuffer.render_pass))
        {
    ...
    void DirectionalLightShadowPass::SetupFramebuffer()
    {
        ...
        if (RHI_SUCCESS != m_RenderCommand->CreateFramebuffer(&framebuffer_create_info, m_FrameBuffer.framebuffer))
        {
    ...
    void DirectionalLightShadowPass::SetupPipelines()
    {
        ...
        RHIDescriptorSetLayout* descriptorset_layouts[] = { m_DescriptorInfos[0].layout, m_per_mesh_layout };
    ...
    void DirectionalLightShadowPass::SetupDescriptorSet()
    {
        ...
        if (RHI_SUCCESS != m_RenderCommand->AllocateDescriptorSets(&mesh_directional_light_shadow_global_descriptor_set_alloc_info, m_DescriptorInfos[0].descriptor_set))
        {
    ...
    void DirectionalLightShadowPass::DrawModel()
    {
        ...
        // Directional Light Shadow begin pass
        {
            ...
            m_RenderCommand->PushEvent(m_RenderCommand->GetCurrentCommandBuffer(), "Directional Light Shadow", color);
        }

        // Mesh
        if (m_RenderCommand->isPointLightShadowEnabled())
        {
            float color[4] = { 1.0f, 1.0f, 1.0f, 1.0f };
            m_RenderCommand->PushEvent(m_RenderCommand->GetCurrentCommandBuffer(), "Mesh", color);
            ...
            for (auto& [material, mesh_instanced] : directional_light_mesh_drawcall_batch)
            {
                ...
                for (auto& [mesh, mesh_nodes] : mesh_instanced)
                {
                    ...
                    if (total_instance_count > 0)
                    {
                        ...
                        m_RenderCommand->CmdBindVertexBuffersPFN(m_RenderCommand->GetCurrentCommandBuffer(), 0, 1, vertex_buffers, offsets);
                        m_RenderCommand->CmdBindIndexBufferPFN(m_RenderCommand->GetCurrentCommandBuffer(), mesh->mesh_index_buffer, 0, RHI_INDEX_TYPE_UINT16);
