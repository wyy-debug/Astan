...
namespace Astan
{
    void FXAAPass::Initialize(const RenderPassInitInfo* init_info)
    {
        ...
        {
            AS_CORE_ERROR("fxaa _init_info is nullptr");
            return;
        }
    ...
    void FXAAPass::setupDescriptorSetLayout()
    {
        ...
        RHIDescriptorSetLayoutBinding& postProcessGlobalLayoutInColor = postProcessGlobalLayoutBindings[0];
        postProcessGlobalLayoutInColor.binding = 0;
        postProcessGlobalLayoutInColor.descriptorType = RHI_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER;
        postProcessGlobalLayoutInColor.descriptorCount = 1;
        postProcessGlobalLayoutInColor.stageFlags = RHI_SHADER_STAGE_FRAGMENT_BIT;
        ...
        postProcessGlobalLayoutCreateInfo.bindingCount =
            sizeof(postProcessGlobalLayoutBindings) / sizeof(postProcessGlobalLayoutBindings[0]);
    ...
    void FXAAPass::setupPipelines()
    {
        ...
        if (RHI_SUCCESS != m_RenderCommand->CreatePipelineLayout(&pipeline_layout_create_info, m_RenderPipelines[0].layout))
        {
            throw std::runtime_error("create post process pipeline layout");
        }

        RHIShader* vert_shader_module = m_RenderCommand->CreateShaderModule(FXAA_VERT);
        RHIShader* frag_shader_module = m_RenderCommand->CreateShaderModule(FXAA_FRAG);
    ...
    void FXAAPass::updateAfterFramebufferRecreate(RHIImageView* input_attachment)
    {
        ...
        post_process_per_frame_input_attachment_info.sampler = m_RenderCommand->GetOrCreateDefaultSampler(Default_Sampler_Linear);
    ...
    void FXAAPass::Draw()
    {
        ...
        m_RenderCommand->PushEvent(m_RenderCommand->GetCurrentCommandBuffer(), "FXAA", color);

        RHIViewport viewport = { 0.0,
                               0.0,
                               static_cast<float>(m_RenderCommand->GetSwapchainInfo().extent.width),
                               static_cast<float>(m_RenderCommand->GetSwapchainInfo().extent.height),
                               0.0,
                               1.0 };
        int32_t    x = static_cast<int32_t>(m_RenderCommand->GetSwapchainInfo().viewport->x);
        int32_t    y = static_cast<int32_t>(m_RenderCommand->GetSwapchainInfo().viewport->y);
        uint32_t   width = static_cast<uint32_t>(m_RenderCommand->GetSwapchainInfo().viewport->width);
        uint32_t   height = static_cast<uint32_t>(m_RenderCommand->GetSwapchainInfo().viewport->height);
        ...
        m_RenderCommand->CmdSetViewportPFN(m_RenderCommand->GetCurrentCommandBuffer(), 0, 1, &viewport);
        m_RenderCommand->CmdSetScissorPFN(m_RenderCommand->GetCurrentCommandBuffer(), 0, 1, &scissor);
