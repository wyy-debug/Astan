...
namespace Astan
{
        ...
        float     z_far)
    {
        ...
        glm::mat4 mat_column = mat;

        // [vec.xyz 1][mat.col0] / [vec.xyz 1][mat.col3] > x_right
        // [vec.xyz 1][mat.col0 - mat.col3*x_right] > 0
        f.m_plane_right = glm::vec4(mat_column[0]) - (glm::vec4(mat_column[3]) * x_right);
        // normalize
        f.m_plane_right *= (1.0 / glm::vec3(f.m_plane_right.x, f.m_plane_right.y, f.m_plane_right.z).length());
        ...
        // length([A B C].xyz)
        f.m_plane_left = (glm::vec4(mat_column[3]) * x_left) - glm::vec4(mat_column[0]);
        // normalize
        f.m_plane_left *= (1.0 / glm::vec3(f.m_plane_left.x, f.m_plane_left.y, f.m_plane_left.z).length());

        // [vec.xyz 1][mat.col1] / [vec.xyz 1][mat.col3] < y_top
        // [vec.xyz 1][mat.col3*y_top - mat.col1] > 0
        f.m_plane_top = (glm::vec4(mat_column[3]) * y_top) - glm::vec4(mat_column[1]);
        // normalize
        f.m_plane_top *= (1.0 / glm::vec3(f.m_plane_top.x, f.m_plane_top.y, f.m_plane_top.z).length());

        // [vec.xyz 1][mat.col1] / [vec.xyz 1][mat.col3] > y_bottom
        // [vec.xyz 1][mat.col1 - mat.col3*y_bottom] > 0
        f.m_plane_bottom = glm::vec4(mat_column[1]) - (glm::vec4(mat_column[3]) * y_bottom);
        // normalize
        f.m_plane_bottom *= (1.0 / glm::vec3(f.m_plane_bottom.x, f.m_plane_bottom.y, f.m_plane_bottom.z).length());

        // [vec.xyz 1][mat.col2] / [vec.xyz 1][mat.col3] < z_near
        // [vec.xyz 1][mat.col3*z_near - mat.col2] > 0
        f.m_plane_near = (glm::vec4(mat_column[3]) * z_near) - glm::vec4(mat_column[2]);
        f.m_plane_near *= (1.0 / glm::vec3(f.m_plane_near.x, f.m_plane_near.y, f.m_plane_near.z).length());

        // [vec.xyz 1][mat.col2] / [vec.xyz 1][mat.col3] > z_far
        // [vec.xyz 1][mat.col2 - mat.col3*z_far] > 0
        f.m_plane_far = glm::vec4(mat_column[2]) - (glm::vec4(mat_column[3]) * z_far);
        f.m_plane_far *= (1.0 / glm::vec3(f.m_plane_far.x, f.m_plane_far.y, f.m_plane_far.z).length());
    ...
    bool TiledFrustumIntersectBox(ClusterFrustum const& f, BoundingBox const& b)
    {
        ...
        // plane_right
        {
            ...
            float radius_project_plane_right =
                glm::dot(glm::vec3(fabs(f.m_plane_right.x), fabs(f.m_plane_right.y), fabs(f.m_plane_right.z)), box_extents);
        ...
        // plane_left
        {
            ...
            float radius_project_plane_left = 
                glm::dot(glm::vec3(fabs(f.m_plane_left.x), fabs(f.m_plane_left.y), fabs(f.m_plane_left.z)), box_extents);
        ...
        // plane_top
        {
            ...
            float radius_project_plane_top =
                glm::dot(glm::vec3(fabs(f.m_plane_top.x), fabs(f.m_plane_top.y), fabs(f.m_plane_top.z)), box_extents);
        ...
        // plane_bottom
        {
            ...
            float radius_project_plane_bottom =
                glm::dot(glm::vec3(fabs(f.m_plane_bottom.x), fabs(f.m_plane_bottom.y), fabs(f.m_plane_bottom.z)), box_extents);
        ...
        // plane_near
        {
            ...
            float radius_project_plane_near =
                glm::dot(glm::vec3(fabs(f.m_plane_near.x), fabs(f.m_plane_near.y), fabs(f.m_plane_near.z)), box_extents);
        ...
        // plane_far
        {
            ...
            float radius_project_plane_far =
                glm::dot(glm::vec3(fabs(f.m_plane_far.x), fabs(f.m_plane_far.y), fabs(f.m_plane_far.z)), box_extents);
