...
namespace Astan
{
    void ParticleEmitterBufferBatch::freeUpBatch(std::shared_ptr<VulkanRendererAPI> rhi)
    {
    ...
    {
        uint8_t index =
            (m_RenderCommand->GetCurrentFrameIndex() + m_RenderCommand->GetMaxFramesInFlight() - 1) % m_RenderCommand->GetMaxFramesInFlight();

        m_RenderCommand->WaitForFencesPFN(1, &(m_RenderCommand->GetFenceList()[index]), VK_TRUE, UINT64_MAX);
        ...
        bool res_begin_command_buffer = m_RenderCommand->BeginCommandBufferPFN(m_copy_command_buffer, &command_buffer_begin_info);
        assert(RHI_SUCCESS == res_begin_command_buffer);

        float color[4] = { 1.0f, 1.0f, 1.0f, 1.0f };
        m_RenderCommand->PushEvent(m_copy_command_buffer, "Copy Depth Image for Particle", color);
        ...
        imagememorybarrier.subresourceRange = subresourceRange;
        {
            ...
            m_RenderCommand->CmdPipelineBarrier(m_copy_command_buffer,
            ...
            m_RenderCommand->CmdCopyImageToImage(m_copy_command_buffer,
                m_src_depth_image,
        ...
        m_RenderCommand->PopEvent(m_copy_command_buffer); // end depth image copy label

        m_RenderCommand->PushEvent(m_copy_command_buffer, "Copy Normal Image for Particle", color);
        ...
        m_RenderCommand->PopEvent(m_copy_command_buffer);

        bool res_end_command_buffer = m_RenderCommand->EndCommandBufferPFN(m_copy_command_buffer);
        assert(RHI_SUCCESS == res_end_command_buffer);

        bool res_reset_fences = m_RenderCommand->ResetFencesPFN(1, &m_RenderCommand->GetFenceList()[index]);
        ...
        submit_info.pWaitSemaphores = &(m_RenderCommand->GetTextureCopySemaphore(index));
        ...
        bool res_queue_submit =
            m_RenderCommand->QueueSubmit(m_RenderCommand->GetGraphicsQueue(), 1, &submit_info, m_RenderCommand->GetFenceList()[index]);
    ...
    {
        m_RenderCommand->DestroyImage(m_dst_depth_image);
        m_RenderCommand->FreeMemory(m_dst_depth_image_memory);

        m_RenderCommand->CreateImage(m_RenderCommand->GetSwapchainInfo().extent.width,
            m_RenderCommand->GetSwapchainInfo().extent.height,
            m_RenderCommand->GetDepthImageInfo().depth_image_format,
        ...
        m_RenderCommand->DestroyImage(m_dst_normal_image);
        m_RenderCommand->FreeMemory(m_dst_normal_image_memory);
        ...
        m_RenderCommand->CreateImageView(m_dst_depth_image,
            m_RenderCommand->GetDepthImageInfo().depth_image_format,
            RHI_IMAGE_ASPECT_DEPTH_BIT,
        ...
        m_RenderCommand->CreateImageView(m_dst_normal_image,
            RHI_FORMAT_R8G8B8A8_UNORM,
            RHI_IMAGE_ASPECT_COLOR_BIT,
            RHI_IMAGE_VIEW_TYPE_2D,
            1,
    ...
    void ParticlePass::Draw()
    {
        ...
        {
            float color[4] = { 1.0f, 1.0f, 1.0f, 1.0f };
            m_RenderCommand->PushEvent(m_render_command_buffer, "ParticleBillboard", color);

            m_RenderCommand->CmdBindPipelinePFN(
                m_render_command_buffer, RHI_PIPELINE_BIND_POINT_GRAPHICS, m_RenderPipelines[1].pipeline);
            m_RenderCommand->CmdSetViewportPFN(m_render_command_buffer, 0, 1, m_RenderCommand->GetSwapchainInfo().viewport);
            m_RenderCommand->CmdSetScissorPFN(m_render_command_buffer, 0, 1, m_RenderCommand->GetSwapchainInfo().scissor);
            m_RenderCommand->CmdBindDescriptorSetsPFN(m_render_command_buffer,
            ...
            m_RenderCommand->CmdDraw(m_render_command_buffer, 4, m_emitter_buffer_batches[i].m_num_particle, 0, 0);

            m_RenderCommand->PopEvent(m_render_command_buffer);
        }
    }
    ...
    void ParticlePass::SetupParticleDescriptorSet()
    {
        for (int eid = 0; eid < m_emitter_count; ++eid)
        {
            ...
            particlebillboard_global_descriptor_set_alloc_info.descriptorPool = m_RenderCommand->GetDescriptorPoor();
            ...
            if (RHI_SUCCESS != m_RenderCommand->AllocateDescriptorSets(&particlebillboard_global_descriptor_set_alloc_info,
            ...
            if (RHI_SUCCESS != m_RenderCommand->CreateSampler(&samplerCreateInfo, sampler))
            {
            ...
            m_RenderCommand->UpdateDescriptorSets(3, particlebillboard_descriptor_writes_info, 0, NULL);
        }
    }
    ...
    void ParticlePass::SetEmitterCount(int count)
    {
        ...
        {
            m_emitter_buffer_batches[i].freeUpBatch(m_RenderCommand);
        }
    ...
    void ParticlePass::CreateEmitter(int id, const ParticleEmitterDesc& desc)
    {
        ...
        {
            AS_CORE_INFO("Emitter {} info:", id);
            AS_CORE_INFO("Dead {}, Alive {}, After sim {}, Emit {}",
                counter.dead_count,
                counter.alive_count,
...

        {
            ...
            m_RenderCommand->CreateBufferAndInitialize(RHI_BUFFER_USAGE_STORAGE_BUFFER_BIT | RHI_BUFFER_USAGE_INDIRECT_BUFFER_BIT,
            ...
            m_RenderCommand->CreateBufferAndInitialize(RHI_BUFFER_USAGE_STORAGE_BUFFER_BIT,
                RHI_MEMORY_PROPERTY_HOST_VISIBLE_BIT,
        ...
        {
            m_RenderCommand->CreateBufferAndInitialize(RHI_BUFFER_USAGE_TRANSFER_SRC_BIT | RHI_BUFFER_USAGE_TRANSFER_DST_BIT,
                RHI_MEMORY_PROPERTY_HOST_VISIBLE_BIT,
            ...
            m_RenderCommand->MapMemory(m_emitter_buffer_batches[id].m_counter_host_memory, 0, RHI_WHOLE_SIZE, 0, &mapped);

            m_RenderCommand->FlushMappedMemoryRanges(
                nullptr, m_emitter_buffer_batches[id].m_counter_host_memory, 0, RHI_WHOLE_SIZE);

            m_RenderCommand->UnmapMemory(m_emitter_buffer_batches[id].m_counter_host_memory);

            m_RenderCommand->CreateBufferAndInitialize(RHI_BUFFER_USAGE_STORAGE_BUFFER_BIT | RHI_BUFFER_USAGE_TRANSFER_SRC_BIT |
            ...
            CmdBufAllocateInfo.commandPool = m_RenderCommand->GetCommandPoor();
            CmdBufAllocateInfo.level = RHI_COMMAND_BUFFER_LEVEL_PRIMARY;
            CmdBufAllocateInfo.commandBufferCount = 1;
            RHICommandBuffer* copyCmd;
            if (RHI_SUCCESS != m_RenderCommand->AllocateCommandBuffers(&CmdBufAllocateInfo, copyCmd))
            {
            ...
            if (RHI_SUCCESS != m_RenderCommand->BeginCommandBuffer(copyCmd, &CmdBufInfo))
            {
            ...
            m_RenderCommand->CmdCopyBuffer(copyCmd,
                m_emitter_buffer_batches[id].m_counter_host_buffer,
                m_emitter_buffer_batches[id].m_counter_device_buffer,
                1,
                &copyRegion);

            if (RHI_SUCCESS != m_RenderCommand->EndCommandBuffer(copyCmd))
            {
            ...
            fenceInfo.flags = 0;
            if (RHI_SUCCESS != m_RenderCommand->CreateFence(&fenceInfo, fence))
            {
                throw std::runtime_error("create fence");
            }

            // Submit to the queue
            if (RHI_SUCCESS != m_RenderCommand->QueueSubmit(m_RenderCommand->GetComputeQueue(), 1, &submitInfo, fence))
            {
                throw std::runtime_error("queue submit");
            }

            if (RHI_SUCCESS != m_RenderCommand->WaitForFencesPFN(1, &fence, RHI_TRUE, UINT64_MAX))
            {
                throw std::runtime_error("wait fence submit");
            }

            m_RenderCommand->DestroyFence(fence);
            m_RenderCommand->FreeCommandBuffers(m_RenderCommand->GetCommandPoor(), 1, copyCmd);
        }
        ...
        // fill in data
        {
            ...
            if (RHI_SUCCESS != m_RenderCommand->MapMemory(m_emitter_buffer_batches[id].m_particle_component_res_memory,
                0,
                RHI_WHOLE_SIZE,
                0,
            ...
            // Flush writes to host visible buffer
            void* mapped;
            m_RenderCommand->MapMemory(m_emitter_buffer_batches[id].m_position_host_memory, 0, RHI_WHOLE_SIZE, 0, &mapped);

            m_RenderCommand->FlushMappedMemoryRanges(
                nullptr, m_emitter_buffer_batches[id].m_position_host_memory, 0, RHI_WHOLE_SIZE);

            m_RenderCommand->UnmapMemory(m_emitter_buffer_batches[id].m_position_host_memory);
    ...
    void ParticlePass::SetupParticlePass()
    {
        ...
        CmdBufAllocateInfo.level = RHI_COMMAND_BUFFER_LEVEL_PRIMARY;
        CmdBufAllocateInfo.commandBufferCount = 1;
        if (RHI_SUCCESS != m_RenderCommand->AllocateCommandBuffers(&CmdBufAllocateInfo, m_compute_command_buffer))
            throw std::runtime_error("alloc compute command buffer");
        if (RHI_SUCCESS != m_RenderCommand->AllocateCommandBuffers(&CmdBufAllocateInfo, m_copy_command_buffer))
        ...
        if (RHI_SUCCESS != m_RenderCommand->CreateFence(&fenceCreateInfo, m_fence))
            throw std::runtime_error("create fence");
    }
    ...
    void ParticlePass::SetupDescriptorSetLayout()
    {
        ...
        // compute descriptor sets
        {
            ...
            if (RHI_SUCCESS !=
                m_RenderCommand->CreateDescriptorSetLayout(&particle_descriptor_layout_create_info, m_DescriptorInfos[0].layout))
            {
                throw std::runtime_error("setup particle compute Descriptor done");
            }
            AS_CORE_INFO("setup particle compute Descriptor done");
        }
        // scene depth and normal binding
        {
            ...
            if (RHI_SUCCESS != m_RenderCommand->CreateDescriptorSetLayout(&gbuffer_lighting_global_layout_create_info,
                m_DescriptorInfos[1].layout))
...

        {
            ...
            if (RHI_SUCCESS != m_RenderCommand->CreateDescriptorSetLayout(&particlebillboard_global_layout_create_info,
    ...
    void ParticlePass::SetupPipelines()
    {
        ...
        // compute pipeline
        {
            RHIDescriptorSetLayout* descriptorset_layouts[2] = { m_DescriptorInfos[0].layout,
                                                                    m_DescriptorInfos[1].layout };
            ...
            AS_CORE_INFO("compute pipe layout done");
        }
        /*
...

        {
            ...
            if (RHI_SUCCESS != m_RenderCommand->CreateComputePipelines(
        ...
        // particle billboard
        {
            ...
            if (m_RenderCommand->CreatePipelineLayout(&pipeline_layout_create_info, m_RenderPipelines[1].layout) != RHI_SUCCESS)
            {
            ...
            if (m_RenderCommand->CreateGraphicsPipelines(RHI_NULL_HANDLE, 1, &pipelineInfo, m_RenderPipelines[1].pipeline) !=
                RHI_SUCCESS)
            {
    ...
    void ParticlePass::AllocateDescriptorSet()
    {
        ...
        particle_descriptor_set_alloc_info.descriptorPool = m_RenderCommand->GetDescriptorPoor();

        m_DescriptorInfos.resize(3 * m_emitter_count);
        for (int eid = 0; eid < m_emitter_count; ++eid)
        {
            ...
            if (RHI_SUCCESS != m_RenderCommand->AllocateDescriptorSets(&particle_descriptor_set_alloc_info,
            ...
            if (RHI_SUCCESS != m_RenderCommand->AllocateDescriptorSets(&particle_descriptor_set_alloc_info,
                ...
                AS_CORE_INFO("allocate normal and depth descriptor set done");
        }
    }
    ...
    void ParticlePass::UpdateDescriptorSet()
    {
        for (int eid = 0; eid < m_emitter_count; ++eid)
        {
            // compute part
            {
                ...
                m_RenderCommand->UpdateDescriptorSets(static_cast<uint32_t>(computeWriteDescriptorSets.size()),
            ...
            }
            {
                ...
                m_RenderCommand->UpdateDescriptorSets(sizeof(descriptor_input_attachment_writes_info) /
    ...
    void ParticlePass::Simulate()
    {
        for (auto i : m_emitter_tick_indices)
        {
            ...
            if (RHI_SUCCESS != m_RenderCommand->BeginCommandBuffer(m_compute_command_buffer, &CmdBufInfo))
            {
                throw std::runtime_error("begin command buffer");
            }

            float color[4] = { 1.0f, 1.0f, 1.0f, 1.0f };
            m_RenderCommand->PushEvent(m_compute_command_buffer, "Particle compute", color);
            m_RenderCommand->PushEvent(m_compute_command_buffer, "Particle Kickoff", color);

            m_RenderCommand->CmdBindPipelinePFN(m_compute_command_buffer, RHI_PIPELINE_BIND_POINT_COMPUTE, m_kickoff_pipeline);
            RHIDescriptorSet* descriptorsets[2] = { m_DescriptorInfos[i * 3].descriptor_set,
                                                   m_DescriptorInfos[i * 3 + 1].descriptor_set };
            m_RenderCommand->CmdBindDescriptorSetsPFN(m_compute_command_buffer,
            ...
            m_RenderCommand->CmdDispatch(m_compute_command_buffer, 1, 1, 1);

            m_RenderCommand->PopEvent(m_compute_command_buffer); // end particle kickoff label
            ...
            m_RenderCommand->CmdPipelineBarrier(m_compute_command_buffer,
            ...
            m_RenderCommand->PushEvent(m_compute_command_buffer, "Particle Emit", color);

            m_RenderCommand->CmdBindPipelinePFN(m_compute_command_buffer, RHI_PIPELINE_BIND_POINT_COMPUTE, m_emit_pipeline);

            m_RenderCommand->CmdDispatchIndirect(m_compute_command_buffer,
                m_emitter_buffer_batches[i].m_indirect_dispatch_argument_buffer,
                s_argument_offset_emit);

            m_RenderCommand->PopEvent(m_compute_command_buffer); // end particle emit label
            ...
            m_RenderCommand->PushEvent(m_compute_command_buffer, "Particle Simulate", color);

            m_RenderCommand->CmdBindPipelinePFN(m_compute_command_buffer, RHI_PIPELINE_BIND_POINT_COMPUTE, m_simulate_pipeline);
            m_RenderCommand->CmdDispatchIndirect(m_compute_command_buffer,
                m_emitter_buffer_batches[i].m_indirect_dispatch_argument_buffer,
                s_argument_offset_simulate);

            m_RenderCommand->PopEvent(m_compute_command_buffer); // end particle simulate label

            if (RHI_SUCCESS != m_RenderCommand->EndCommandBuffer(m_compute_command_buffer))
            {
                throw std::runtime_error("end command buffer");
            }
            m_RenderCommand->ResetFencesPFN(1, &m_fence);
            ...
            if (RHI_SUCCESS != m_RenderCommand->QueueSubmit(m_RenderCommand->GetComputeQueue(), 1, &computeSubmitInfo, m_fence))
            {
                throw std::runtime_error("compute queue submit");
            }

            if (RHI_SUCCESS != m_RenderCommand->WaitForFencesPFN(1, &m_fence, RHI_TRUE, UINT64_MAX))
            {
            ...
            m_RenderCommand->PushEvent(m_compute_command_buffer, "Copy Particle Counter Buffer", color);
            ...
            m_RenderCommand->CmdCopyBuffer(m_compute_command_buffer,
            ...
            m_RenderCommand->PopEvent(m_compute_command_buffer); // end particle counter copy label

            m_RenderCommand->PopEvent(m_compute_command_buffer); // end particle compute label
            ...
            m_RenderCommand->QueueWaitIdle(m_RenderCommand->GetComputeQueue());

            // Make device writes visible to the host
            void* mapped;
            m_RenderCommand->MapMemory(m_emitter_buffer_batches[i].m_counter_host_memory, 0, RHI_WHOLE_SIZE, 0, &mapped);

            m_RenderCommand->InvalidateMappedMemoryRanges(
            ...
            m_RenderCommand->UnmapMemory(m_emitter_buffer_batches[i].m_counter_host_memory);
    ...
    void ParticlePass::PrepareUniformBuffer()
    {
        ...
        m_RenderCommand->CreateBuffer(sizeof(m_particle_collision_perframe_storage_buffer_object),
            RHI_BUFFER_USAGE_UNIFORM_BUFFER_BIT,
            RHI_MEMORY_PROPERTY_HOST_VISIBLE_BIT | RHI_MEMORY_PROPERTY_HOST_COHERENT_BIT,
            m_scene_uniform_buffer,
            d_mem);

        if (RHI_SUCCESS != m_RenderCommand->MapMemory(d_mem, 0, RHI_WHOLE_SIZE, 0, &m_scene_uniform_buffer_mapped))
        {
            throw std::runtime_error("map billboard uniform buffer");
        }
        RHIDeviceMemory* d_uniformdmemory;

        m_RenderCommand->CreateBufferAndInitialize(RHI_BUFFER_USAGE_UNIFORM_BUFFER_BIT,
            RHI_MEMORY_PROPERTY_HOST_VISIBLE_BIT | RHI_MEMORY_PROPERTY_HOST_COHERENT_BIT,
            m_compute_uniform_buffer,
            d_uniformdmemory,
            sizeof(m_ubo));

        if (RHI_SUCCESS != m_RenderCommand->MapMemory(d_uniformdmemory, 0, RHI_WHOLE_SIZE, 0, &m_particle_compute_buffer_mapped))
        {
        ...
        {
            RHIDeviceMemory* d_mem;
            m_RenderCommand->CreateBuffer(sizeof(m_particlebillboard_perframe_storage_buffer_object),
                RHI_BUFFER_USAGE_UNIFORM_BUFFER_BIT,
                RHI_MEMORY_PROPERTY_HOST_VISIBLE_BIT | RHI_MEMORY_PROPERTY_HOST_COHERENT_BIT,
            ...
            if (RHI_SUCCESS !=
                m_RenderCommand->MapMemory(d_mem, 0, RHI_WHOLE_SIZE, 0, &m_particle_billboard_uniform_buffer_mapped))
            {
                throw std::runtime_error("map billboard uniform buffer");
            }
        }
    ...
    void ParticlePass::UpdateUniformBuffer()
    {
        ...
        m_ubo.viewport.z = m_RenderCommand->GetSwapchainInfo().viewport->width;
        m_ubo.viewport.w = m_RenderCommand->GetSwapchainInfo().viewport->height;
        m_ubo.extent.x = m_RenderCommand->GetSwapchainInfo().scissor->extent.width;
        m_ubo.extent.y = m_RenderCommand->GetSwapchainInfo().scissor->extent.height;
