...
namespace Astan
{
    ...
    public:
        ...
        virtual void UploadGlobalRenderResource(Ref<RendererAPI> rhi,
        ...
        virtual void UpdatePerFrameBuffer(Ref<Scene>  render_scene,
            Ref<EditorCamera> camera) override final;
    ...
    private:
        void CreateAndMapStorageBuffer(Ref<RendererAPI> rhi);
        void CreateIBLSamplers(Ref<RendererAPI> rhi);
        void CreateIBLTextures(Ref<RendererAPI>                        rhi,
            std::array<Ref<TextureData>, 6> irradiance_maps,
            std::array<Ref<TextureData>, 6> specular_maps);

        VulkanMesh& GetOrCreateVulkanMesh(Ref<RendererAPI> rhi, RenderEntity entity, RenderMeshData mesh_data);
        VulkanPBRMaterial&
            GetOrCreateVulkanMaterial(Ref<RendererAPI> rhi, RenderEntity entity, RenderMaterialData material_data);

        void UpdateMeshData(Ref<RendererAPI>                          rhi,
            ...
            VulkanMesh& now_mesh);
        void UpdateVertexBuffer(Ref<RendererAPI>                          rhi,
            bool                                          enable_vertex_blending,
            uint32_t                                      vertex_buffer_size,
            struct MeshVertexDataDefinition const* vertex_buffer_data,
            uint32_t                                      joint_binding_buffer_size,
            ...
            VulkanMesh& now_mesh);
        void UpdateIndexBuffer(Ref<RendererAPI> rhi,
            uint32_t             index_buffer_size,
            void* index_buffer_data,
            VulkanMesh& now_mesh);
        void UpdateTextureImageData(Ref<RendererAPI> rhi, const TextureDataToUpdate& texture_data);
    };
} // namespace Piccolo
