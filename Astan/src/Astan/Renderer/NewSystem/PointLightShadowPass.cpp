...
namespace Astan
{
    ...
    void PointLightShadowPass::Draw()
    {
        ...
        m_RenderCommand->PushEvent(m_RenderCommand->GetCurrentCommandBuffer(), "Point Light Shadow", color);
    ...
    void PointLightShadowPass::DrawModel()
    {
        ...
        if (m_RenderCommand->isPointLightShadowEnabled())
        {
            ...
            m_RenderCommand->CmdBindPipelinePFN(
                m_RenderCommand->GetCurrentCommandBuffer(), RHI_PIPELINE_BIND_POINT_GRAPHICS, m_RenderPipelines[0].pipeline);

            // perframe storage buffer
            uint32_t perframe_dynamic_offset =
                roundUp(m_global_render_resource->_storage_buffer._global_upload_ringbuffers_end[m_RenderCommand->GetCurrentFrameIndex()],
            ...
            assert(m_global_render_resource->_storage_buffer._global_upload_ringbuffers_end[m_RenderCommand->GetCurrentFrameIndex()] <=
                (m_global_render_resource->_storage_buffer._global_upload_ringbuffers_begin[m_RenderCommand->GetCurrentFrameIndex()] +
                    m_global_render_resource->_storage_buffer._global_upload_ringbuffers_size[m_RenderCommand->GetCurrentFrameIndex()]));
            ...
            for (auto& pair1 : point_lights_mesh_drawcall_batch)
            {
                ...
                for (auto& pair2 : mesh_instanced)
                {
                    ...
                    if (total_instance_count > 0)
                    {
                        ...
                        m_RenderCommand->CmdBindVertexBuffersPFN(
                            m_RenderCommand->GetCurrentCommandBuffer(), 0, 1, vertex_buffers, offsets);
                        m_RenderCommand->CmdBindIndexBufferPFN(
                            m_RenderCommand->GetCurrentCommandBuffer(), mesh.mesh_index_buffer, 0, RHI_INDEX_TYPE_UINT16);
