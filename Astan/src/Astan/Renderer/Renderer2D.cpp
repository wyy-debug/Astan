...
namespace Astan
{
	...
	void Renderer2D::Init() 
	{
		...
		s_Data->WhiteShader = Texture2D::Create(1, 1);
		uint32_t whiteTextureData = 0xffffffff;
		s_Data->WhiteShader->SetData(&whiteTextureData,sizeof(uint32_t));
	...
	void Renderer2D::DrawQuad(const glm::vec3& positon, const glm::vec2& size, const glm::vec4& color)
	{
		...
		glm::mat4 transform = glm::translate(glm::mat4(1.0f), positon) * /*Roation*/
	...
	void Renderer2D::DrawQuad(const glm::vec2& positon, const glm::vec2& size, const Ref<Texture2D>& texture) 
	{
		DrawQuad({ positon.x,positon.y,0.0f }, size, texture);
	}
