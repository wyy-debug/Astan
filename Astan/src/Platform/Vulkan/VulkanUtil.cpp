...
namespace Astan
{
        ...
        VkMemoryPropertyFlags properties_flag)
    {
        ...
        AS_CORE_ERROR("findMemoryType error");
        return 0;
    }
        ...
        int                   datasize)
    {
        ...
        {
            AS_CORE_ERROR("create buffer buffer failed!");
            return;
        }
        ...
        if (!memTypeFound)
        {
            AS_CORE_ERROR("memTypeFound is nullptr");
            return;
        }
        if (VK_SUCCESS != vkAllocateMemory(device, &memAlloc, nullptr, memory))
        {
            AS_CORE_ERROR("alloc memory failed!");
            return;
        }
        ...
        if (data != nullptr && datasize != 0)
        {
            ...
            {
                AS_CORE_ERROR("map memory failed!");
                return;
            }
        ...
        {
            AS_CORE_ERROR("bind memory failed!");
            return;
        }
    }
        ...
        VkDeviceMemory& buffer_memory)
    {
        ...
        {
            AS_CORE_ERROR("vkCreateBuffer failed!");
            return;
        }
        ...
        {
            AS_CORE_ERROR("vkAllocateMemory failed!");
            return;
        }
    ...
    void VulkanUtil::copyBuffer(RendererAPI* rhi,
        VkBuffer     srcBuffer,
        VkBuffer     dstBuffer,
        ...
        VkDeviceSize size)
    {
        if (rhi == nullptr)
        {
            AS_CORE_ERROR("rhi is nullptr");
            return;
        }

        RHICommandBuffer* rhi_command_buffer = static_cast<VulkanRendererAPI*>(rhi)->BeginSingleTimeCommands();
        ...
        static_cast<VulkanRendererAPI*>(rhi)->EndSingleTimeCommands(rhi_command_buffer);
    }
        ...
        uint32_t              miplevels)
    {
        ...
        {
            AS_CORE_ERROR("failed to create image!");
            return;
        }
        ...
        {
            AS_CORE_ERROR("failed to allocate image memory!");
            return;
        }
    ...
    void VulkanUtil::createGlobalImage(RendererAPI* rhi,
        VkImage& image,
        VkImageView& image_view,
        ...
        uint32_t           miplevels)
    {
        ...
        case RHIFormat::RHI_FORMAT_R32G32B32A32_SFLOAT:
            ...
            break;
        default:
            AS_CORE_ERROR("invalid texture_byte_size");
            break;
        }
        ...
        VulkanUtil::createBuffer(static_cast<VulkanRendererAPI*>(rhi)->m_physical_device,
            static_cast<VulkanRendererAPI*>(rhi)->m_device,
            texture_byte_size,
        ...
        void* data;
        vkMapMemory(
            static_cast<VulkanRendererAPI*>(rhi)->m_device, inefficient_staging_buffer_memory, 0, texture_byte_size, 0, &data);
        memcpy(data, texture_image_pixels, static_cast<size_t>(texture_byte_size));
        vkUnmapMemory(static_cast<VulkanRendererAPI*>(rhi)->m_device, inefficient_staging_buffer_memory);
        ...
        vkDestroyBuffer(static_cast<VulkanRendererAPI*>(rhi)->m_device, inefficient_staging_buffer, nullptr);
        vkFreeMemory(static_cast<VulkanRendererAPI*>(rhi)->m_device, inefficient_staging_buffer_memory, nullptr);

        // generate mipmapped image
        genMipmappedImage(rhi, image, texture_image_width, texture_image_height, mip_levels);

        image_view = createImageView(static_cast<VulkanRendererAPI*>(rhi)->m_device,
            image,
            vulkan_image_format,
            VK_IMAGE_ASPECT_COLOR_BIT,
    ...
    void VulkanUtil::createCubeMap(RendererAPI* rhi,
        VkImage& image,
        VkImageView& image_view,
        VmaAllocation& image_allocation,
        uint32_t             texture_image_width,
        ...
        uint32_t             miplevels)
    {
        ...
        default:
            ...
            AS_CORE_ERROR("invalid texture_layer_byte_size");
            return;
            break;
        }
        ...
        VkBuffer       inefficient_staging_buffer;
        VkDeviceMemory inefficient_staging_buffer_memory;
        createBuffer(static_cast<VulkanRendererAPI*>(rhi)->m_physical_device,
            static_cast<VulkanRendererAPI*>(rhi)->m_device,
            cube_byte_size,
            VK_BUFFER_USAGE_TRANSFER_SRC_BIT,
            VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT,
        ...
        void* data = NULL;
        vkMapMemory(
            static_cast<VulkanRendererAPI*>(rhi)->m_device, inefficient_staging_buffer_memory, 0, cube_byte_size, 0, &data);
        for (int i = 0; i < 6; i++)
        {
    ...
    void VulkanUtil::generateTextureMipMaps(RendererAPI* rhi,
        VkImage  image,
        ...
        uint32_t miplevels)
    {
        ...
        vkGetPhysicalDeviceFormatProperties(
            static_cast<VulkanRendererAPI*>(rhi)->m_physical_device, image_format, &format_properties);
        if (!(format_properties.optimalTilingFeatures & VK_FORMAT_FEATURE_SAMPLED_IMAGE_FILTER_LINEAR_BIT))
        {
            AS_CORE_ERROR("generateTextureMipMaps() : linear bliting not supported!");
            return;
        }
    ...
    void VulkanUtil::transitionImageLayout(RendererAPI* rhi,
        VkImage            image,
        ...
        VkImageAspectFlags aspect_mask_bits)
    {
        ...
        {
            AS_CORE_ERROR("unsupported layout transition!");
            return;
        }
    ...
    void VulkanUtil::copyBufferToImage(RendererAPI* rhi,
        VkBuffer buffer,
        VkImage  image,
        uint32_t width,
        uint32_t height,
    ...
    void VulkanUtil::genMipmappedImage(RendererAPI* rhi, VkImage image, uint32_t width, uint32_t height, uint32_t mip_levels)
    {
        if (rhi == nullptr)
        {
            AS_CORE_ERROR("rhi is nullptr");
    ...
    VkSampler VulkanUtil::getOrCreateNearestSampler(VkPhysicalDevice physical_device, VkDevice device)
    {
        if (m_nearest_sampler == VK_NULL_HANDLE)
        {
            ...
            {
                AS_CORE_ERROR("vk create sampler");
            }
        }
